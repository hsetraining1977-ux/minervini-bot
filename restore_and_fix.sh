...
# ============================================================
# restore_and_fix.sh
# 1. يستعيد health_dashboard.py من النسخة الاحتياطية
# 2. يطبق threshold 15 دقيقة بشكل صحيح
# 3. يعيد تشغيل dashboard 8501
# ============================================================

echo "============================================"
echo "  Restore & Fix — health_dashboard.py"
echo "============================================"
echo ""

TARGET="/root/health_dashboard.py"

# ── 1. إيجاد أحدث نسخة احتياطية ─────────────────────────────
LATEST_BAK=$(ls -t ${TARGET}.bak_* 2>/dev/null | head -1)
...
if [ -n "$LATEST_BAK" ]; then
    ...
    echo "⚠️  No backup found — will write fresh file"
fi

# ── 2. كتابة الملف الصحيح كاملاً ───────────────────────────
cat > "$TARGET" << 'PYEOF'
"""
...
echo "✅ health_dashboard.py written fresh (threshold = 15 min)"
echo ""

# ── 3. تحقق من الكود ──────────────────────────────────────────
python3 -c "import ast; ast.parse(open('/root/health_dashboard.py').read()); print('✅ Syntax OK')"
echo ""

# ── 4. إيقاف المنفذ 8501 ─────────────────────────────────────
echo "Stopping port 8501..."
pkill -f "streamlit.*8501" 2>/dev/null
pkill -f "health_dashboard" 2>/dev/null
sleep 2
...
nohup /root/.local/bin/streamlit run /root/health_dashboard.py \
    --server.port 8501 \
...
if ps -p $NEW_PID > /dev/null 2>&1; then
    echo "✅ Dashboard 8501 running (PID: $NEW_PID)"
else
    echo "❌ Failed — last 10 lines of log:"
    tail -10 /root/logs/dashboard_8501.log
fi
...
echo "  Threshold: 15 minutes (Fresh < 15m)"
echo "============================================"
